...
## A. 기본 개념
### A-1. Iterable 객체
* 정의: 반복 가능한 객체. `for` 루프를 사용해서 그 안의 요소들을 하나씩 꺼낼 수 있는 객체 의미
* 예시: 리스트(list), 튜플(tuple), 딕셔너리(dict), 세트(set), 문자열(str) 등이 대표적인 이터러블 객체
* 특징:
> * 내부에 __iter__() 라는 특별한 메소드를 가지고 있음
> * Iterator와 Generator는 일회용 (Exhaustible)
>> * `for`루프 등으로 모든 값을 소진하고 나면 이터레이터는 "비어 있는" 상태가 됨.
>> * 다시 순회하려면 `iter(iterable_object)`를 호출하여 새로운 이터레이터를 생성해야 함
* 예시
```
# 이들은 모두 이터러블합니다.
my_list = [1, 2, 3]
...
### A-2. Iterator
* 정의: 값을 차례대로 꺼낼 수 있는 상태를 가진 객체. 어디까지 값을 꺼냈는지 기억하고 있다가, 다음 값을 요청하면 그 다음 값을 반환
* 특징:
> * __iter__() 메소드를 가짐. (주로 self 반환)
> * __next__() 메소드를 가짐. 이 메소드를 호출할 때마다 다음 값 반환
> * 더 이상 반환할 값이 없으면 StopIteration 예외 발생`
* 생성 방법: 이터러블 객체에 `iter()` 내장 함후를 호출하여 얻을 수 있음
```
my_list = [1, 2, 3]
...
|메모리|전체 결과가 리스트로 저장|필요할 때마다 하나씩 생성|
...
# 여러 시퀀스를 연결
combined = chain(range(3), ['A','B'])
print(list(combined))  # [0,1,2,'A','B']
# 처음 5개만 슬라이스
nums = (i*i for i in range(100))
print(list(islice(nums, 5)))  # [0,1,4,9,16]
```
...
* 예외와 리소스 관리를 위해 try/finally 내에서 close() 호출하거나 컨텍스트 매니저 래핑 패턴 사용
* 요약: 제너레이터는 Pythonic하고 메모리 효율적인 반복 스트림 처리 기법이며, yield와 yield from으로 복잡한 파이프라인까지 구현 가능합니다.
