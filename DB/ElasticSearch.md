...
> * 서로 다른 인덱스라도 검색할 필드명만 같다면 여러 인덱스를 한 번에 조회할 수 있는 기능 제공
* __도큐먼트 지향(Document-Oriented)__
> * 여러 계층 구조의 문서를 JSON 형식으로 저장할 수 있으며, 이러한 문서도 한 번의 쿼리로 쉽게 조회할 수 있
* __역색인(Inverted Index)__
> * 모든 단어가 어떤 문서에 포함되어 있는지를 색인하여, 특정 단어가 포함된 문서를 빠르게 검색할 수 있
* __확장성__
...
> * 이는 내부적으로 커밋(commit), 플러시(flush)와 같은 복잡한 과정을 거치기 때문임
* __트랜잭션과 롤백 기능 미지원__
> * 전체적인 클러스터의 성능 향상을 위해 비용 소모가 큰 트랜잭션과 롤백 기능을 지원하지 않음
* __데이터 업데이트의 비효율성__
> * 엘라스틱서치는 문서를 직접 수정하지 않으며, 업데이트 명령이 올 경우 기존 문서를 삭제하고 새로운 문서를 생성하는 방식으로 처리함
...
> * 데이터베이스는 업데이트 이전의 저널 파일을 사용하여 원래의 정상적인 상태로 되돌
> * 이는 오류 발생 이후에도 깨끗한 사본으로 복원시킬 수 있기 때문에, 무결성을 위해 중요
* __트랜잭션(Transaction)__
> * 데이터베이스의 상태를 변환시키는 하나의 논리적 기능을 수행하기 위한 작업의 단위 또는 한 번에 모두 수행되어야 할 일련의 연산을 의미
> * 트랜잭션은 데이터베이스 시스템에서 병행 제어 및 회복 작업 시 처리되는 작업의 논리적 단위이며, 사용자가 시스템에 대한 서비스 요구 시 시스템이 응답하기 위한 상태 변환 과정의 작업 단위임
...
* 엘라스틱서치는 기본적으로 HTTP 프로토콜을 통해 접근이 가능한 RESTful API를 제공하며, 이를 통해 데이터 조작을 수행함
...
![이미지 2](./images/ElasticS_기본용어.png)
...
* __역색인(Inverted index)__
> * __모든 단어가 어떤 Document에 있는지 색인하는 구조__: 엘라스틱서치는 역색인 구조를 통해 각 단어가 포함된 문서를 빠르게 찾을 수 있습니다.
> * __대소문자 구분 없이 검색 가능__: 엘라스틱서치는 텍스트를 분석하여 대소문자 구분 없이 검색을 수행하며, 형태소 분석 등을 통해 다양한 형태의 단어도 검색할 수 있습니다.
> * 역색인의 예시

![이미지 3](./images/ElasticS_역섹인.png)
